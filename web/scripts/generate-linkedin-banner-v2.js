...
const LLMS = [
  { name: 'ChatGPT', file: 'chatgpt.png', hasLogo: true },
  { name: 'Claude', file: 'claude.png', hasLogo: true },
  { name: 'Gemini', color: '#8b5cf6', hasLogo: false },
  { name: 'Perplexity', color: '#20b2aa', hasLogo: false },
  { name: 'Grok', file: 'grok.png', hasLogo: true },
  { name: 'DeepSeek', file: 'deepseek.png', hasLogo: true },
  { name: 'Copilot', color: '#0078d4', hasLogo: false },
  { name: 'Meta AI', color: '#0668E1', hasLogo: false }
];
...
async function generateBanner() {
  ...
  ctx.beginPath();
  ctx.arc(WIDTH * 0.08, HEIGHT * 0.5, 120, 0, Math.PI * 2);
  ctx.stroke();
  ...
  const spacing = (WIDTH * 0.70) / (LLMS.length - 1);
  const y = HEIGHT * 0.80;
  const logoSize = 32;
  ...
  for (let i = 0; i < LLMS.length; i++) {
    ...
    if (llm.hasLogo) {
      try {
        ...
        ctx.beginPath();
        ctx.arc(x, y - 5, logoSize / 2 + 4, 0, Math.PI * 2);
        ctx.fill();

        // Dibujar logo
        ctx.save();
        ctx.beginPath();
        ctx.arc(x, y - 5, logoSize / 2 + 2, 0, Math.PI * 2);
        ctx.clip();
        ctx.drawImage(logo, x - logoSize / 2, y - 5 - logoSize / 2, logoSize, logoSize);
        ctx.restore();
      } catch (e) {
        // Fallback a círculo de color
        ctx.fillStyle = '#666';
        ctx.beginPath();
        ctx.arc(x, y - 5, logoSize / 2, 0, Math.PI * 2);
        ctx.fill();
      }
    } else {
      // Círculo con color
  ...
  ctx.lineTo(startX + (LLMS.length - 1) * spacing, y - 5);
  ctx.stroke();

  // Guardar
  const outputPath = path.join(__dirname, '..', 'public', 'images', 'social', 'linkedin-banner-nicolas-v2.jpg');
  const buffer = canvas.toBuffer('image/jpeg', { quality: 0.95 });
  fs.writeFileSync(outputPath, buffer);

  console.log('✅ Banner V2 generado: linkedin-banner-nicolas-v2.jpg');
}
